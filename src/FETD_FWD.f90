	...
	!   ! sur_edg_num sur_edg_list
	!allocate(tri_on_bound(tri_num),plane_judge(cmod%nod_num))
	!tri_on_bound = .false.
	!do i=1,tri_num
	!	vec1(1) = cmod%nod_cor(tri_tmp(i,2),1)-cmod%nod_cor(tri_tmp(i,1),1)
	!	vec1(2) = cmod%nod_cor(tri_tmp(i,2),2)-cmod%nod_cor(tri_tmp(i,1),2)
	!	vec1(3) = cmod%nod_cor(tri_tmp(i,2),3)-cmod%nod_cor(tri_tmp(i,1),3)
	!	vec2(1) = cmod%nod_cor(tri_tmp(i,3),1)-cmod%nod_cor(tri_tmp(i,2),1)
	!	vec2(2) = cmod%nod_cor(tri_tmp(i,3),2)-cmod%nod_cor(tri_tmp(i,2),2)
	!	vec2(3) = cmod%nod_cor(tri_tmp(i,3),3)-cmod%nod_cor(tri_tmp(i,2),3)
	!    plane(1:3) = cross_p (vec1,vec2)
	!	plane(4) = -(plane(1)*cmod%nod_cor(tri_tmp(i,1),1)+plane(2)*cmod%nod_cor(tri_tmp(i,1),2)+plane(3)*cmod%nod_cor(tri_tmp(i,1),3))
	!	do j=1,cmod%nod_num
	!		plane_judge(j) = plane(1)*cmod%nod_cor(j,1)+plane(2)*cmod%nod_cor(j,2)+plane(3)*cmod%nod_cor(j,3)+plane(4)
	!	end do
	!	if (all(plane_judge<1.0e-5).or.all(plane_judge>(-1.0e-5))) tri_on_bound(i)=.true.
	!end do
	!   allocate(cmod%sur_edg_list(cmod%edg_num))
	!   cmod%sur_edg_list = .false.
	!   do i=1,tri_num
	!	if(tri_on_bound(i)) then
	!       do j=1,3
	!           vex1 = tri_tmp(i,ind_t2d(j,1))
	!           vex2 = tri_tmp(i,ind_t2d(j,2))
	!           call swap(vex1,vex2,vex1>vex2)
	!           tmp = bSearch_Ii ( cmod%EDG_VEX(:,1), cmod%EDG_VEX(:,2), vex1, vex2)
	!           cmod%sur_edg_list(tmp) = .true.
	!	end do
	!	end if
	!   end do
	!   cmod%sur_edg_num = count(cmod%sur_edg_list)
	!allocate(cmod%sur_edg_index(cmod%sur_edg_num))
	!j=0
	!do i=1,cmod%edg_num
	!	if(cmod%sur_edg_list(i)) then
	!		j=j+1
	!	    cmod%sur_edg_index(j)=i
	!	end if
	!end do
	! edg_len
	...
	!integer :: m_size
	!integer,allocatable :: m_ROW(:),m_COL(:)
	!REAL(8),ALLOCATABLE :: m_VAL(:)
	!type(SPARSE_MATRIX_T) :: mt_mkl,m_mkl
	!INTEGER :: nsys,nnzsys
	!INTEGER,ALLOCATABLE :: isys(:),jsys(:)
	!REAL(8),ALLOCATABLE :: vsys(:),RHS(:)
	!real(8),allocatable :: phi_element(:,:),phi_edge(:,:)
	!integer,allocatable :: phi_edg_num(:)
	!! process
	!if(output_level) print*, "--- generate DC solution at step 0 ..."
	!ALLOCATE(m_row(16*cmod%tet_NUM),m_col(16*cmod%tet_NUM),m_val(16*cmod%tet_NUM))
	!m_SIZE=0; m_row=0; m_col=0; m_val=0.d0
	!do i=1,cmod%tet_num
	!	! loc_m
	!	loc_m = 0.d0
	!	do j=1,4
	!		do k=1,4
	!			if (cmod%iso) then
	!				loc_m(J,K) = 1.d0 / 36.d0 / cmod%tet_vol(i) * cmod%sig_3d(i) * (cmod%b(i,j)*cmod%b(i,k)+cmod%c(i,j)*cmod%c(i,k)+cmod%d(i,j)*cmod%d(i,k))
	!			else
	!				loc_vec = matmul(cmod%sig_a3d(i,:,:),[cmod%b(i,k),cmod%c(i,k),cmod%d(i,k)])
	!				loc_m(J,K) = 1.d0 / 36.d0 / cmod%tet_vol(i) * (cmod%b(i,j)*loc_vec(1)+cmod%c(i,j)*loc_vec(2)+cmod%d(i,j)*loc_vec(3))
	!			end if
	!		end do
	!	end do
	...
	!	end do
	!end do
	!info = mkl_sparse_d_create_coo (mt_mkl, SPARSE_INDEX_BASE_one, cmod%nod_num, cmod%nod_num, m_SIZE, m_ROW(1:m_SIZE), m_COL(1:m_SIZE), m_VAL(1:m_SIZE))
	!info = mkl_sparse_convert_csr (mt_mkl, SPARSE_OPERATION_NON_TRANSPOSE, m_mkl)
	!! convert to csr
	!call mkl_to_csr (M_mkl,nsys,nnzsys,vsys,isys,jsys)
	!! RHS
	!ALLOCATE(RHS(cmod%nod_num),init_FAI(cmod%nod_num))
	!RHS = 0.D0
	!!DO I=1,CMOD%seg_num
	!!    !inner: do j=1,cmod%tet_num
	!!    !    do k=1,6
	!!    !        if (cmod%src_edg(i)==cmod%tet_edg(j,k)) then
	!!    !            itet = j
	!!    !            exit inner
	!!    !        end if
	!!    !    end do
	!!    !end do inner
	!!    RHS(CMOD%EDG_VEX(cmod%src_edg(i),1)) = 1.d0*1.33333
	!!    RHS(CMOD%EDG_VEX(cmod%src_edg(i),2)) = 1.d0*1.33333
	!!    !do j=1,4
	!!    !    if (CMOD%EDG_VEX(cmod%src_edg(i),1)==cmod%tet_vex(itet,j)) then
	!!    !        !print*,cmod%tet_vol(itet) / 4.d0 / 2.d0 / CMOD%seg_num
	!!    !        RHS(CMOD%EDG_VEX(cmod%src_edg(i),1)) = RHS(CMOD%EDG_VEX(cmod%src_edg(i),1)) + cmod%tet_vol(itet) / 4.d0 / 2.d0 / CMOD%seg_num / 2.e15
	!!    !    end if
	!!    !    if (CMOD%EDG_VEX(cmod%src_edg(i),2)==cmod%tet_vex(itet,j)) then
	!!    !        !print*,cmod%tet_vol(itet) / 4.d0 / 2.d0 / CMOD%seg_num
	!!    !        RHS(CMOD%EDG_VEX(cmod%src_edg(i),2)) = RHS(CMOD%EDG_VEX(cmod%src_edg(i),2)) + cmod%tet_vol(itet) / 4.d0 / 2.d0 / CMOD%seg_num / 2.e15
	!!    !    end if
	!!    !end do
	!!END DO
	!RHS(CMOD%EDG_VEX(cmod%src_edg(1),1)) = 1.d0 * cmod%src_current / cmod%seg_num / 2.d0
	!RHS(CMOD%EDG_VEX(cmod%src_edg(CMOD%seg_num),2)) = -1.d0 * cmod%src_current / cmod%seg_num / 2.d0
	!!do i=1,cmod%nod_num
	!!    if (rhs(i)/=0) print*,i,rhs(i)
	!!end do
	!!print*,count(rhs/=0)
	!! SOLVE THE SYSTEM
	!call pardiso_direct_1 (.true.,nsys,nnzsys,isys,jsys,vsys)
	!call pardiso_direct_2 (RHS,init_FAI)
	!call pardiso_direct_3
	!! GENERATE EDGE E FIELD ACCORDING TO NODE POTENTIAL
	!!allocate(phi_element(cmod%tet_num,3))
	!!phi_element=0.d0
	!!do i=1,cmod%tet_num
	!!    phi_element(i,1) = 1.d0 / 6.d0 / cmod%tet_vol(i) * (cmod%b(i,1)*init_FAI(cmod%tet_vex(i,1))+cmod%b(i,2)*init_FAI(cmod%tet_vex(i,2))+cmod%b(i,3)*init_FAI(cmod%tet_vex(i,3))+cmod%b(i,4)*init_FAI(cmod%tet_vex(i,4)))
	!!    phi_element(i,2) = 1.d0 / 6.d0 / cmod%tet_vol(i) * (cmod%c(i,1)*init_FAI(cmod%tet_vex(i,1))+cmod%c(i,2)*init_FAI(cmod%tet_vex(i,2))+cmod%b(i,3)*init_FAI(cmod%tet_vex(i,3))+cmod%c(i,4)*init_FAI(cmod%tet_vex(i,4)))
	!!    phi_element(i,3) = 1.d0 / 6.d0 / cmod%tet_vol(i) * (cmod%d(i,1)*init_FAI(cmod%tet_vex(i,1))+cmod%d(i,2)*init_FAI(cmod%tet_vex(i,2))+cmod%b(i,3)*init_FAI(cmod%tet_vex(i,3))+cmod%d(i,4)*init_FAI(cmod%tet_vex(i,4)))
	!!end do
	!!allocate(phi_edge(cmod%tet_num,6))
	!!phi_edge=0.d0
	!!do i=1,cmod%tet_num
	!!    do j=1,6
	!!        loc_vec(1) = cmod%nod_cor(maxval([cmod%tet_vex(i,ind_tet(j,1)),cmod%tet_vex(i,ind_tet(j,2))]),1) - cmod%nod_cor(minval([cmod%tet_vex(i,ind_tet(j,1)),cmod%tet_vex(i,ind_tet(j,2))]),1)
	!!        loc_vec(2) = cmod%nod_cor(maxval([cmod%tet_vex(i,ind_tet(j,1)),cmod%tet_vex(i,ind_tet(j,2))]),2) - cmod%nod_cor(minval([cmod%tet_vex(i,ind_tet(j,1)),cmod%tet_vex(i,ind_tet(j,2))]),2)
	...
	!!        phi_edge(i,j) = dot_product([phi_element(i,1),phi_element(i,2),phi_element(i,3)],loc_vec) / sqrt(loc_vec(1)**2.d0+loc_vec(2)**2.d0+loc_vec(3)**2.d0)
	!!    end do
	!!end do
	!allocate(INIT_E(CMOD%EDG_NUM),phi_edg_num(CMOD%EDG_NUM))
	!!INIT_E = 0.d0
	!!phi_edg_num = 0
	!!do i=1,cmod%tet_num
	!!    do j=1,6
	!!        phi_edg_num(cmod%tet_edg(i,j)) = phi_edg_num(cmod%tet_edg(i,j))+1
	!!        INIT_E(cmod%tet_edg(i,j)) = INIT_E(cmod%tet_edg(i,j))+phi_edge(i,j)
	!!    end do
	!!end do
	!!do i=1,CMOD%EDG_NUM
	!!    INIT_E(i)=INIT_E(i)/phi_edg_num(i)
	!!end do
	!DO I=1,CMOD%EDG_NUM
	!	INIT_E(I) = ( init_FAI(CMOD%edg_vex(I,2)) - init_FAI(CMOD%edg_vex(I,1)) ) / CMOD%edg_len(I)
	!END DO
		...
		if ( mod(step,cmod%step_dble)==0 ) then
			...
			if(rank==0)	then
				...
				if (error>1.0e-4.and.not_changed<2.and.job=="f") then
					...
					e2 = e_veri_1
                    !step = step + 1
                    time_cur = tmp_time_cur! - step_cur
	...
	! output to opt_unit
	write(opt_unit,"(i6,e15.5,2x,"//itoa(cmod%rec_num)//"("//itoa(count(cmod%o_f))//"e15.5,2x))") step,t,(cmod%a_f(cmod%ind_f,i,cmod%o_v),i=1,cmod%rec_num)
	...
	if (allocated(cmod%time_channels)) then
		do i=1,cmod%time_channel_num
			if (t>=cmod%time_channels(i).and.cmod%time_channel_ok(i)==.false.) then
				...
				write(opt2_unit,"(i6,e15.5,2x,"//itoa(cmod%rec_num)//"("//itoa(count(cmod%o_f))//"e15.5,2x))") i,cmod%time_channels(i),(cmod%a_f_channel(i,j,cmod%o_v),j=1,cmod%rec_num)
