	...
	if(narg==2) then
		...
		write(*,*) 'Usage: ./TEMF3DT -[job] FILE_ROOT'
		write(*,*) '[FORWARD]		-f	Calculates the predicted data and saves the EM solution'
		write(*,*) '[INVERSION]		-i	Generate pre-inversion information'
		write(*,*) '[COMPUTE_J]		-j	Calculates and saves the full J(acobian)'
		write(*,*) '[MULT_BY_J]		-m	Multiplies a model by J to create a data vector'
		write(*,*) '[MULT_BY_J_T]	-t	Multiplies a data vector by J^T to create a model'
	...
	!write(*,*), 'TEMF3DT: Run job -',job,' in folder ',buf(1:ilen),' with root name: ',TRIM(file_root)
	! set file names
